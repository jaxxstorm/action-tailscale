...
async function run() {
    try {
        ...
        let finalChannel = 'stable'; // default
        let finalVersion = '';
        if (channelInput) {
        ...
        if (sha256sum) {
            ...
            if (actualSha256 !== sha256sum.replace(/\s+/g, '')) {
                throw new Error(`SHA256 mismatch! Expected=${sha256sum}, got=${actualSha256}`);
            }
        }
        else {
...
async function fetchLatestFromChannel(channel) {
    const http = new http_client_1.HttpClient('action-tailscale');
...
async function startEphemeralTailscaled(daemonPath, stateDir, extraArgs) {
    ...
    try {
        ...
        core.warning(`tailscaled may not be fully started yet: ${err.message}`);
    }
}
/**
...
function getTailscaledBinaryName(os) {
